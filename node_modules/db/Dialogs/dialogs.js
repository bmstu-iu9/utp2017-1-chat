...
var dialogsPath = __dirname + '/dialogs.json';
var dialogs;

var newsPath = __dirname + '/news.json';

var imagesPath = "./temp";

exports.connect = function() {
    try {
        dialogs = JSON.parse(fs.readFileSync(dialogsPath, 'utf-8'));
    } catch (err) {
        if (err.message.indexOf("ENOENT") == 0) {
            log.info("Создание файла с комнатами");
            fs.writeFileSync(dialogsPath, JSON.stringify({}, '', 4), 'utf-8');
        } else
            log.error(err);
        dialogs = {};
    }
...
exports.getMessages = function(room, start, isLastMessages) {
    return new Promise(function(resolve, reject) {
        ...
        fs.readFile(dialogsPath, 'utf-8', function(err, db) {
            if (err) {
                log.error(err);
                reject(err);
        ...
        fs.readFile(dialogsPath, 'utf-8', function(err, db) {
            if (err) {
                ...
                for (var i = 0; i < Object.keys(db).length; i++) {
                    if (db[Object.keys(db)[i]].title)
                        x.push({author: db[Object.keys(db)[i]].author,
                        id: db[Object.keys(db)[i]].id, title: db[Object.keys(db)[i]].title});
                    else  x.push({author: db[Object.keys(db)[i]].author,
...
exports.addRoom = function(author, title) {
    return new Promise(function(resolve, reject) {
        log.debug("Добавление комнаты " + title);
        fs.readFile(dialogsPath, 'utf-8', function(err, db) {
            if (err) {
                log.error(err);
                return reject(err);
            ...
            fs.writeFile(dialogsPath, db, 'utf-8', function(err) {
                if (err) {
                    log.error(err);
                    reject(err);
                } else
                    resolve(JSON.stringify({author: author, id: room, title: title}));
            });
        });
    });
};
...
exports.deleteAllRooms = function() {
    ...
    fs.readdir(imagesPath, function (err, files) {
        if (err) {
            ...
            for (const file of files) {
                fs.unlink(imagesPath + file, function (err) {
                    if (err)
    ...
    fs.writeFile(dialogsPath, JSON.stringify({}, '', 4), 'utf-8', function(err) {
        if (err)
            throw new Error(err);
    });
...
exports.getNews = function(geolocation) {
    return new Promise(function(resolve, reject) {

        fs.readFile(newsPath, 'utf-8', function(err, db) {
            if (err) {
                if (err.message.indexOf("ENOENT") == 0) {
                    log.info("Создание файла с новостями");
                    fs.writeFileSync(newsPath, JSON.stringify({}, '', 4), 'utf-8');
                    db = [];
                } else {
                    log.error(err);
                    reject(err);
                ...
                db = JSON.parse(db);
                var x = [];
                x.push( {header: "", text: "Ваша широта: " + geolocation.latitude + " | Ваша Долгота: " + geolocation.longitude, date: new Date().toDateString()} )
                for (var i = 0; i < Object.keys(db).length; i++)
